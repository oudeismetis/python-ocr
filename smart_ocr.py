...
PSM_AUTO_ONLY = 2  # Automatic page segmentation, but no OSD
...
PSM_RAW_LINE = 13  # Treat the image as a single text line, skipping any pre-processing
PSM_COUNT = 14 # Number of enum entries
ALL_PSM_MODES = [PSM_OSD_ONLY, PSM_AUTO_OSD, PSM_AUTO_ONLY, PSM_AUTO, 
...
def clean_up_noise(processed_images, psm_mode):
    """Enhanced noise cleanup with book-specific preprocessing"""
    try:
        all_text = []
        for i, img in enumerate(processed_images):
            # Configure Tesseract for better book text recognition
            custom_config = f"--oem 3 --psm {psm_mode} -c tessedit_char_whitelist={WHITELIST}"
            # with tempfile.TemporaryDirectory() as temp_dir:
            #     text = image_to_string(img, config=custom_config + f' -c user_defined_dpi=300 --tessdata-dir {temp_dir}')
                # text = image_to_string(img, config=custom_config)
            pil_img = Image.fromarray(img)
            text = image_to_string(pil_img, config=custom_config)
            if text.strip():
...
def main(img_name):
    ...
    text = simple_ocr(img_name)
    if text and len(text.strip()) > 20:  # If we get reasonable amount of text
        print("=== Simple OCR Success ===")
        with open(f"{output_folder}/simple_output.txt", "w") as f:
            f.write(text)
    # Use the enhanced preprocessing pipeline
    processed_images = preprocess_book_image(img_name, f"{output_folder}/images/")
    for psm_mode in ALL_PSM_MODES:
        text = clean_up_noise(processed_images, psm_mode)
        if text:
            with open(f"{output_folder}/text/{psm_mode}_output.txt", "w") as f:
                f.write(text)
